...
ggplot(data=GasPrices) +
  ...
  xlabs('Mean Income Per Area') +
  ylabs('Price of Gas Per Gallon')
```
...
ggplot(data=GasPrices.Brand) +
  ...
  xlabs('Average Price of Gas Per Gallon')
```
...
sclass_test_350 = testing(sclass_split_350)
  this_rmse_350 = foreach(k = k_grid, .combine='c') %do% {
    ...
    knn_model = knnreg(price ~ mileage, data=sclass_test_350, k = k, use.all=TRUE)
    modelr::rmse(knn_model, sclass_train_350)
  }
...
```{r echo=FALSE}
# Clean data for 63 AMG
sclass_63AMG = sclass %>%
  filter(trim == '63 AMG') %>%
  select(c('trim','price', 'mileage'))

#Plot the data for 63 AMG
ggplot(data = sclass_63AMG) +
  geom_point(aes(x=mileage, y=price))

#Create training and testing datasets
k_grid = c(2, 4, 6, 8, 10, 15, 20, 25, 30, 35, 40, 45, 50)
rmse_out_63AMG = foreach(i=1:length(k_grid), .combine='rbind') %dopar% {
sclass_split_63AMG = initial_split(sclass_63AMG, prop = 0.9)
sclass_train_63AMG = training(sclass_split_63AMG)
sclass_test_63AMG = testing(sclass_split_63AMG)
  this_rmse_63AMG = foreach(k = k_grid, .combine='c') %do% {
    # train the model and calculate RMSE on the test set
    knn_model = knnreg(price ~ mileage, data=sclass_test_63AMG, k = k, use.all=TRUE)
    modelr::rmse(knn_model, sclass_train_63AMG)
  }
  data.frame(k=k_grid, rmse=this_rmse_63AMG)
}
rmse_out_63AMG = arrange(rmse_out_63AMG, k)
```

```{r echo=FALSE}
#Plot K versus RMSE for 63 AMG

ggplot(rmse_out_63AMG) + geom_boxplot(aes(x=factor(k), y=rmse)) + theme_bw(base_size=7)

```

```{r echo=FALSE}
#KNN with k=20
knn20 = knnreg(price ~ mileage, data = sclass_train_63AMG, k=20) #In the caret package
rmse = rmse(knn20, sclass_train_63AMG)
rmse

#Plot the fit -> Attach predictions to the test data frame
sclass_test_63AMG = sclass_test_63AMG %>%
  mutate(sclass_pred = predict(knn20, sclass_test_63AMG))

p_test_63AMG = ggplot(data = sclass_test_63AMG) +
  geom_point(mapping = aes(x=mileage, y=price), alpha = 0.2)
p_test_63AMG

#Add predictions
p_test_63AMG + geom_line(aes(x=mileage, y=sclass_pred), color='red', size=1.5)
```
