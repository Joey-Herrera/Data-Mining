...
sclass_split = initial_split(sclass_350, prop = 0.9)
sclass_train = training(sclass_split)
sclass_test = testing(sclass_split)

#KNN with k=3
knn3 = knnreg(price ~ mileage, data = sclass_train, k=3) #In the caret package
rmse = rmse(knn3, sclass_train)
rmse

#Plot the fit -> Attach predictions to the test data frame
sclass_test = sclass_test %>%
  mutate(sclass_pred = predict(knn3, sclass_test))

p_test = ggplot(data = sclass_test) +
  geom_point(mapping = aes(x=mileage, y=price), alpha = 0.2)
p_test

#Add predictions
p_test + geom_line(aes(x=mileage, y=sclass_pred), color='red', size=1.5)
...
#Create training and testing datasets
sclass_split = initial_split(sclass_63AMG, prop = 0.9)
sclass_train = training(sclass_split)
sclass_test = testing(sclass_split)
